...
int send_fd(const ACE_HANDLE socket, int fd_to_send)
{
...
int recv_fd(const ACE_HANDLE socket)
{
...
int main()
{
	...
	cout<<"Connected Successfully to the Server!\n"<<endl;

    int fd = recv_fd(sfd.get_handle());
    int sz;
    if(fd > 0)
    {
        cout<<"File Descriptor Received Successfully"<<endl;
        cout<<"Reading and printing the file\n"<<endl;
